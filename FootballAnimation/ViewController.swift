...
class ViewController: UIViewController {
    ...
    var viewSize = CGRect.zero
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        viewSize = background?.bounds ?? CGRect.zero
        
        guard let bundle = Bundle.main.path(forResource: "goals", ofType: "json") else { return }
        let items = JSONParser.parse(url: URL(fileURLWithPath: bundle))
        drawPlay(play: items.first)
    }
}
...
extension ViewController {
    ...
    func drawPlay(play: Play?) {
        ...
        for player in play.players {
            ...
            if let p = player.position.first {
                if let lp = lastPlace {
                    background?.layer.addSublayer(pass(coordinateStart:lp, p))
                }
            }
            ...
            for c in player.position {
                ...
                if let lr = lastRun {
                    background?.layer.addSublayer((run(coordinateStart:lr, coordinateEnd:c)))
                }
                
                background?.addSubview(playerCircle(coordinate:c, number: String(player.number), alpha: alpha))
                alpha = alpha + 0.5
                lastRun = c
            }
    ...
    func playerCircle(coordinate:String, number:String, alpha:Double = 1.0) -> UIView {
    ...
    func run(coordinateStart:String, coordinateEnd:String) -> CAShapeLayer {
        return line(coordinateStart: coordinateStart, coordinateEnd: coordinateEnd, type: Line.run)
    }
    
    func pass(coordinateStart:String, _ coordinateEnd:String) -> CAShapeLayer {
        return line(coordinateStart: coordinateStart, coordinateEnd: coordinateEnd, type: Line.pass)
    }
    
    func line(coordinateStart:String, coordinateEnd:String, type:Line) -> CAShapeLayer {
